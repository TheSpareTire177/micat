...
- Use Ferium/libium functionality to search, scan, and download mods and modpacks from major hosts (e.g., Modrinth, CurseForge).
- Place downloaded content into the bound `/data` directories of the `mc-permanent` and `mc-seasonal` containers so they pick up the appropriate packs on restart.
...
- Utilize Docker and Docker Compose to:
  - Define and run the two Minecraft server containers (`mc-permanent` and `mc-seasonal`) using the `itzg/minecraft-server` image.
  - Bind host directories (managed by Micat) into each container's `/data` directory so Micat can control worlds, configs, and mods.
...
- uses docker compose to set up these containers for use and management
